...
import tech.hombre.freelancehunt.BuildConfig
import java.util.*
...
class AutoStartPermissionHelper private constructor() {
    ...
    private fun launchOnePlusInfo(context: Context): Boolean {
        return try {
            val i = Intent("android.settings.APP_BATTERY_SETTINGS").apply {
    ...
    private fun autoStartOnePlus(context: Context): Boolean {
        if (isPackageExists(context, PACKAGE_ONE_PLUS_MAIN)) {
            try {
                ...
                e.printStackTrace()
                return launchOnePlusInfo(context)
            }
        } else {
            return launchOnePlusInfo(context)
        }
