...
// called before last frame (1f).
    ...
    private fun setupAnimators() {
        ...
        animator.addListener(object : Animator.AnimatorListener {
            ...
            override fun onAnimationEnd(animator: Animator) { // do nothing
            }
            ...
            override fun onAnimationRepeat(animator: Animator) {
                ...
                ring.storeOriginals()
                ring.goToNextColor()
                if (mFinishing) { // finished closing the last ring from the swipe gesture; go
// into progress mode
                    mFinishing = false
                    animator.cancel()
...
// animating, the mColorIndex moves by one to the next available color.
        ...
        fun draw(c: Canvas, bounds: Rect) {
            ...
            if (centerRadius <= 0) { // If center radius is not set, fill the bounds
                arcRadius =
