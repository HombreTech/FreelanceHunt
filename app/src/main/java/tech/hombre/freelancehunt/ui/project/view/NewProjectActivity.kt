...
class NewProjectActivity : BaseActivity() {
    ...
    private fun initViews() {
            ...
            val picker = DatePickerDialog(
                ...
                DatePickerDialog.OnDateSetListener { view, year, month, dayOfMonth ->
                    ...
                    endDateButton.text = String.format(
                        "%s/%s/%s",
                        if (dd > 9) dd else "0$dd",
                        if (mm > 9) mm else "0$mm",
                        yy
                    )
        ...
        val dd: Int = calendar.get(Calendar.DAY_OF_MONTH)
        endDateButton.text =
            String.format("%s/%s/%s", if (dd > 9) dd else "0$dd", if (mm > 9) mm else "0$mm", yy)
        val format = SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ssXXX", Locale.getDefault())
        endDate = format.format(calendar.time)
    ...
    private fun correctInputs(): Boolean {
        return endDate.isNotEmpty() && !projectTitle.text.isNullOrEmpty() && description.savedText.isNotEmpty() && !budgetValue.text.isNullOrEmpty() && checkedSkills.any { it }
    }
